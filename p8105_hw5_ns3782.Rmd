...
The homicides dataset contains information about homicides in multiple U.S. cities. The dataset has `r nrow(homicide_data)` homicide cases with `r ncol(homicide_data)` variables: `r names(homicide_data)`. The cases span from `r min(homicide_data$reported_date)` through `r max(homicide_data$reported_date)`. The data covers `r n_distinct(homicide_data$city_state)` cities.

The victim demographics include age (ranging from `r min(homicide_data$victim_age)` to `r max(homicide_data$victim_age)` years), sex (`r unique(homicide_data$victim_sex)`), and race (`r unique(homicide_data$victim_race)`). Case dispositions fall into the following categories: `r unique(homicide_data$disposition)`. Each case has geographic coordinates (latitude and longitude) and a unique identifier (uid).
